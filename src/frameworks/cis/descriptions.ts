...
export interface SafeguardDescription {
  ...
  requiresComparativeSnapshot?: boolean // true for delta-based controls that need comparison
  ...
  '3': {
    ...
    safeguards: [
      ...
      },
      {
        ...
        dataSources: [
          { name: 'Managed IPv4', path: '/technology/addressing/managed-ip/ipv4' }
        ],
        target: '100%',
  ...
  '4': {
    ...
    safeguards: [
      ...
      },
      {
        ...
        dataSources: [
          { name: 'DNS Servers', path: '/technology/management/dns/servers' }
        ],
        target: 'Delta ≥ 0',
  ...
  '5': {
    ...
    safeguards: [
      ...
      },
      {
        ...
        dataSources: [
          { name: 'AAA Servers', path: '/technology/management/aaa/servers' }
        ],
        target: 'Delta ≥ 0',
    ...
    safeguards: [
      {
        ...
        description: 'Establish and maintain an inventory of the enterprise\'s authentication and authorization systems, including those hosted on-site or at a remote service provider. Centralize access control for all enterprise assets through a directory service or SSO provider, where supported.',
        ipFabricContext: 'TACACS and RADIUS servers configured on device (excl. cloud)',
        ipFabricInfo: 'Shows configured credential management configurations across network devices. Organizations can verify proper authentication mechanisms and identify weak authentication methods. TACACS+ provides separation of AAA functions while RADIUS combines authentication and authorization.',
        dataSources: [
          { name: 'AAA Servers', path: '/technology/management/aaa/servers' },
          { name: 'AAA Authentication', path: '/technology/management/aaa/authentication' }
        ],
        target: 'Delta ≥ 0',
        targetInterpretation: 'Centralized authentication is critical for access control. Increasing AAA adoption demonstrates improved security posture. Only a subset of devices support TACACS+/RADIUS (traditional route/switch), with many supporting newer centralized, federated, or web-based SSO/SAML models.',
        scoringRationale: 'Max 10 points based on AAA percentage coverage. If Delta ≥ 0 or no previous data: (AAA% / 100) × 10 points. If Delta < 0: 0 points (decreased AAA adoption indicates regression).',
        maxPoints: 10,
  ...
  '12': {
    ...
    safeguards: [
      ...
      },
      {
        ...
        dataSources: [
          { name: 'Topology Tree', path: '/diagrams/topology-tree' }
        ],
        target: 'N/A',
    ...
    safeguards: [
      {
        ...
        description: 'Monitor IPv4 route counts to detect network incidents. Route changes can indicate routing attacks, misconfigurations, or network instability.',
        ipFabricContext: 'Number of IPv4 Routes',
        ipFabricInfo: 'Aggregate route table counts across the network. Reachability is paramount for IP networks. Sudden decreases in route counts may indicate connectivity loss, route hijacking, or BGP incidents. Increases may indicate route leaks or unauthorized route announcements.',
        dataSources: [
          { name: 'IPv4 Routes', path: '/technology/routing/routes' }
        ],
        target: 'Delta ≥ 0',
        targetInterpretation: 'Route counts should remain stable or increase. Decreases may indicate connectivity incidents or routing attacks requiring investigation.',
        scoringRationale: 'If Delta ≥ 0 (route count stable or increased): 4 points. If Delta < 0 (routes decreased): 0 points. This detects potential routing incidents.',
        maxPoints: 4,
        requiresComparativeSnapshot: true
      },
      {
        ...
        description: 'Monitor route stability to detect network incidents. Recently converged routes indicate routing protocol churn or instability.',
        ipFabricContext: 'IPv4 Route Stability (recently converged routes)',
        ipFabricInfo: 'Tracks recently converged routes across the network. Unstable routes indicate BGP flapping, OSPF adjacency issues, or network instability that could impact service availability and may indicate an attack or misconfiguration.',
        dataSources: [
          { name: 'Route Stability', path: '/technology/routing/route-stability' }
        ],
        target: '0',
        targetInterpretation: 'Zero unstable routes indicates a stable network. Any unstable routes may indicate incidents requiring investigation.',
        scoringRationale: 'If 0 unstable routes: 2 points. If any unstable routes: 0 points. Route instability can indicate security incidents or misconfigurations.',
        maxPoints: 2
      },
      {
        id: '17.9c',
        name: 'Establish and Maintain Security Incident Thresholds - ErrDisabled Interfaces',
        description: 'Monitor err-disabled interfaces to detect security incidents. Err-disabled states often indicate security violations like port security breaches, BPDU guard triggers, or storm control activations.',
        ipFabricContext: 'Interfaces in err-disabled state',
        ipFabricInfo: 'Interfaces in err-disabled state indicate security or configuration issues. Common causes include port security violations (MAC address flapping), BPDU guard triggers (rogue switches), storm control (broadcast storms), and DHCP snooping violations.',
        dataSources: [
          { name: 'Interfaces', path: '/inventory/interfaces' }
        ],
        target: '0%',
        targetInterpretation: 'Zero err-disabled interfaces is ideal. Each err-disabled interface may indicate a security violation or incident.',
        scoringRationale: 'Score = (100 - err-disabled%) × 0.04. Max 4 points at 0% err-disabled. Higher percentages reduce the score proportionally.',
        maxPoints: 4,
        reversePolarity: true
      }
    ]
