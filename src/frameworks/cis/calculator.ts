...
import type { CISMetrics, CISControl, CISControlDetail } from './types'
import {
  DiscoveryErrors,
  VersionConsistency,
  EndOfSupportSummary,
...
export async function calculateCISControlsProgressive(
  ...
  // Batch 1: Foundation Controls (1, 2, 3) - Sequential (required for foundation data)
  ...
  // Batches 2, 3, 4: Parallel Execution for Performance
  // Fetch all remaining batches simultaneously - rate limiter will queue them appropriately
  const batchResults = await Promise.allSettled([
    fetchBatch2Metrics(snapshotId, totalDevices, apiCall),
    fetchBatch3Metrics(snapshotId, totalDevices, apiCall),
    fetchBatch4Metrics(snapshotId, apiCall)
  ])

  // Extract results, handle failures gracefully
  const [batch2Result, batch3Result, batch4Result] = batchResults

  const batch2Metrics = batch2Result.status === 'fulfilled' ? batch2Result.value : {}
  const batch3Metrics = batch3Result.status === 'fulfilled' ? batch3Result.value : {}
  const batch4Metrics = batch4Result.status === 'fulfilled' ? batch4Result.value : {}

  // Log any failures for debugging
  if (batch2Result.status === 'rejected') {
    console.error('[CIS] Batch 2 failed:', batch2Result.reason)
  }
  if (batch3Result.status === 'rejected') {
    console.error('[CIS] Batch 3 failed:', batch3Result.reason)
  }
  if (batch4Result.status === 'rejected') {
    console.error('[CIS] Batch 4 failed:', batch4Result.reason)
  }

  // Merge all metrics at once
  accumulatedMetrics = {
    ...accumulatedMetrics,
    ...batch2Metrics,
    ...batch3Metrics,
  ...
  // Create controls and fire callbacks sequentially (for progressive UI updates)
    ...
    const enrichedDetails = currentControl.details?.map(currentDetail => {
      ...
      const getCurrentNumericValue = (val: any): number | null => {
        ...
        if (typeof val === 'string') {
          // Skip compound values (multiple numbers = can't compute single delta)
          // e.g., "Zone FW: 10, ANY/ANY: 0" has 2 numbers - skip these
          // Use /\d+\.?\d*/g to match decimal numbers properly (e.g., "46.7" as one match, not "46" and "7")
